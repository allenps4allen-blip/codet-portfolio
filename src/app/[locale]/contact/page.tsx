                ...
                <form onSubmit={handleSubmit} className="glass-card space-y-6 rounded-2xl p-6 sm:p-8">
                  ...
                  {selectedService === "website" && (
                    <div className="overflow-hidden transition-all duration-300" style={{ animation: "slideDown 0.3s ease-out" }}>
                      ...
                      <p className="mb-3 text-sm font-medium">{t("form.followUp.branding")}</p>
                  ...
                  {selectedService === "ai" && (
                    <div className="overflow-hidden transition-all duration-300" style={{ animation: "slideDown 0.3s ease-out" }}>
                      ...
                      <p className="mb-3 text-sm font-medium">{t("form.followUp.channels")}</p>
                      <div className="flex flex-wrap gap-4">
                        {(["channelWebsite", "channelWhatsApp", "channelTelegram", "channelInstagram"] as const).map((key) => (
                          <label key={key} className="flex items-center gap-2 cursor-pointer text-sm text-foreground/70">
                            <input type="checkbox" name="channels" value={key.replace("channel", "").toLowerCase()} className="accent-emerald-500 rounded" />
                            {t(`form.followUp.${key}`)}
                          </label>
                        ))}
                      </div>
                    ...
                    <div className="overflow-hidden transition-all duration-300" style={{ animation: "slideDown 0.3s ease-out" }}>
                      ...
                      <div>
                        <label htmlFor="tools" className="mb-2 block text-sm font-medium">
                          {t("form.followUp.tools")}
                        </label>
                        <input
                          type="text"
                          id="tools"
                          name="tools"
                          placeholder={t("form.followUp.toolsPlaceholder")}
                          className="w-full rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-sm text-foreground placeholder:text-foreground/30 focus:border-white/20 focus:outline-none focus:ring-0 transition-colors"
                        />
                      </div>
                    </div>
